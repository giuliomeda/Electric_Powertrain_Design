...
if strcmp(MD.Stator.Type, 'External')
  ParSign = 1;
  Dgap = MD.Stator.Geometry.InnerDiameter;
  Dnogap = MD.Stator.Geometry.OuterDiameter;
elseif strcmp(MD.Stator.Type, 'Internal')
  ParSign = -1;
  Dgap = MD.Stator.Geometry.OuterDiameter;
  Dnogap = MD.Stator.Geometry.InnerDiameter;
end
...
if ~isfield(MD.Stator.Geometry, 'FirstSlotAngle')
  MD.Stator.Geometry.FirstSlotAngle = 360/MD.Stator.Geometry.Slots/2;
end
...
SlotRadius = (Dgap + MD.Stator.Geometry.SlotHeight * ParSign)/2;
SlotOpeningRadius = (Dgap + MD.Stator.Geometry.SlotOpeningHeight * ParSign)/2;
...
for ii = 1 : MD.Stator.Geometry.Slots/SymFactor
  ...
  % slot material
  x_slot = SlotRadius*cos(alphas * (ii-1) + MD.Stator.Geometry.FirstSlotAngle*pi/180);
  y_slot = SlotRadius*sin(alphas * (ii-1) + MD.Stator.Geometry.FirstSlotAngle*pi/180);
  ...
  x_slotopening = SlotOpeningRadius*cos(alphas * (ii-1) + MD.Stator.Geometry.FirstSlotAngle*pi/180);
  y_slotopening = SlotOpeningRadius*sin(alphas * (ii-1) + MD.Stator.Geometry.FirstSlotAngle*pi/180);
